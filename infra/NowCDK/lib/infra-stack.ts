...
export class InfraStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    function addResourceToMethod(lambdaToAdd: lambda.Function, baseResource: apigateway.Resource, methodName: string ): apigateway.Resource{
    ...
    const neo4jUserParameter = new ssm.StringParameter(this, "neo4jUserParameter", {
      parameterName: "neo4jUser",
      stringValue: "neo4jUser"
    })

    const neo4jPasswordParameter = new ssm.StringParameter(this, "neo4jPasswordParameter", {
      parameterName: "neo4jPassword",
      stringValue: "neo4jPassword"
    })

    const neo4jUriParameter = new ssm.StringParameter(this, "neo4jUriParameter", {
    ...
    const dbPasswordParameter = new ssm.StringParameter(this, "dbPasswordParameter", {
    ...
    spotActivityTopic.addSubscription( new subscriptions.SqsSubscription(schedulePatternSQS))
    ...
    spotsOnlineLambdaRole.addToPolicy(new iam.PolicyStatement({
      actions:[
        "sns:*"
      ],
    ...
    const spotsOnlineLambdaMethod = addResourceToMethod(spotsOnlineLambda, rootMethod, 'online')

    // Spot schedule lambda
    const spotsShcheduledLambdaRole = new iam.Role(this, 'spotsShcheduledLambdaRole', {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      managedPolicies: [
        iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaBasicExecutionRole"),
        iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaRole")
      ],
      roleName: "spotsShcheduledLambdaRole",  
      description: 'Spots Shchedule Lambda Role',
    });

    spotsShcheduledLambdaRole.addToPolicy(new iam.PolicyStatement({
      actions:[
        "sns:*"
      ],
      effect: iam.Effect.ALLOW,
    ...
    const spotsShcheduledLambda = new lambda.Function(this, 'spotsShcheduledLambda', {
      runtime : lambda.Runtime.GO_1_X,
      handler: 'main',
      role: spotsShcheduledLambdaRole, 
      code: lambda.Code.fromAsset(path),
      functionName: "SpotsShcheduleService",
      environment: { // TODO -> How can we manage this env variables ?
        neo4jUser: neo4jUserParameter.parameterName,
        neo4jPassword: neo4jPasswordParameter.parameterName,
        neo4jUri: neo4jUriParameter.parameterName,
    ...
    // const spotsShcheduledMethod = addMethodToApiGateway(spotsShcheduledLambda, mainApiGateway, "spots/scheduled")
    const spotsShcheduledMethod = addResourceToMethod(spotsShcheduledLambda, rootMethod, 'scheduled')
    Tags.of(spotsShcheduledLambda).add("Type",SERVICE_TYPE)
    Tags.of(spotsShcheduledLambda).add("Family",SPOT_FAMILY)
    // Handler
    ...
    const spotHandlerLambda = new lambda.Function(this, 'spotHandlerLambda', {
      ...
      environment: { // TODO -> How can we manage this env variables ?
        ...
        spotsScheduledServiceURL: `https://${mainApiGateway.restApiId}.execute-api.${this.region}.amazonaws.com/prod${spotsShcheduledMethod.path}`
      }
    })
    const spotsHandlerMethod = addResourceToMethod(spotHandlerLambda, rootMethod, 'handler')
    Tags.of(spotHandlerLambda).add("Type",SERVICE_TYPE)
    Tags.of(spotHandlerLambda).add("Family",SPOT_FAMILY)


    // Confirmation Spot Service
    const confirmationSpotLambdaRole = new iam.Role(this, 'confirmationSpotLambdaRole', {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      managedPolicies: [
        iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaBasicExecutionRole"),
        iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaRole")
    ...
    const confirmationSpotLambda = new lambda.Function(this, 'confirmationSpotLambda', {
      runtime : lambda.Runtime.GO_1_X,
      handler: 'main',
    ...
    addMethodToApiGateway(confirmationSpotLambda, mainApiGateway, "confirmation")
    ...
    const scheduledPatternsCheckerRole = new iam.Role(this, 'scheduledPatternsCheckerRole', {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      managedPolicies: [
        iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaBasicExecutionRole"),
        iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaRole")
    ...
    const scheduledPatternsChecker = new lambda.Function(this, 'scheduledPatternsChecker', {
      runtime : lambda.Runtime.GO_1_X,
      handler: 'main',
    ...
    const scheduledPatternsCheckerEvent = new lambdaEvent.SqsEventSource(schedulePatternSQS,{
      enabled: true,
      batchSize: 10
    })
    ...
    const scheduledPatternsCheckerCron = new events.Rule(this, "scheduledPatternsCheckerCron", {
      enabled: true,
